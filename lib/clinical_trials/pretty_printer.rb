  ...
  class PrettyPrinter
    ...
    def dump_xml_from_nct(nct_file = nil)
     ...
     trials.each do |trial|
        ...
        next unless ClinicalTrials::ScrippsNctTrial.check_format(nct_num)
      ...
      class Imedris < Base
        def initialize(path, file = "")
         ...
         File.open(File.join(@path, "imedris_raw.csv"), "w") do |f|
           @nct = ClinicalTrials::ScrippsImedrisList.new(file)
           f.puts @nct.dump
         end
         File.open(File.join(@path, "imedris_processed_trial.txt"), "w") do |f|
           trials = ClinicalTrials::ScrippsImedrisList.get_trials(file)
           trials.each {|trial|
      ...
      class Nct < Base
       def initialize(path, file = "")
         ...
         File.open(File.join(@path, "nct_raw.csv"), "w") do |f|
           @nct = ClinicalTrials::ScrippsNctList.new(file)
           f.puts @nct.dump
         end
         File.open(File.join(@path, "nct_processed_trial.txt"), "w") do |f|
           @trials = ClinicalTrials::ScrippsNctList.get_trials(file)
           @trials.each {|trial|
            f.puts trial.dump.to_yaml
           }
